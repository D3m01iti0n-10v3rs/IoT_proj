// ========== TO DO LIST ==========
// add manual/auto load control mode
// add error handling for emptyFingerprintData()
// switch fingerID++ to GUI 
// fix void timer2Run(uint64_t t)
...
Servo door;
bool doorState, fan1State, fan2State, light1State, light2State, enrollFlag, enrollDone, delFingerprintData, enrollError = false; // States are edited only twice by firmware in 1 class session, the rest is managed by GUI
int student_number;
...
void setup() {
  ...
  timerAlarm(timer2, 3000000, true, 0); // 3 second, auto-reload
  timerStop(timer2);
  timerWrite(timer2, 0);
...
void read_temp_hum(void) {
  ...
  float temp = (float)temperatureByte;
  float hum = (float)humidityByte;

  int photo = analogRead(LDRpin);
      ...
      if (firebaseData.dataType() == "boolean") {
        ...
        if (currentAttendance == false){ // Entering
          ...
          if (student_number == 1){ // Last to leave, loads turn off
            ...
            Firebase.setInt(firebaseData, "/current/studentNumber", student_number);
            Firebase.setString(firebaseData, "/students/" + results + "/time_of_attendance", "null");
...
void emptyFingerprintData() {
  ...
  // Parse the response
  if (Serial2.available() >= 12) { // Header(2) + Address(4) + PID(1) + Length(2) + Confirmation(1) + Checksum(2)
    // Read header
    if (Serial2.read() == 0xEF && Serial2.read() == 0x01) {
      // Skip address
      for (int i = 0; i < 4; i++) {
      ...
      // Read length
      Serial2.read(); // High byte
      Serial2.read(); // Low byte
