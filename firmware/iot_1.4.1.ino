...
void setup() {
  ...
  if (Firebase.getInt(firebaseData, "/track/studentNumber"))
  student_number = firebaseData.intData();
  if (student_number > 0) inSession = true; else inSession = false;
      ...
      if (firebaseData.dataType() == "boolean") {
        ...
        if (currentAttendance == false){ // Entering
          if (student_number == 0){ // First to arrive, loads turn on, door unlocks
            ...
            Firebase.setInt(firebaseData, "/track/studentNumber", student_number);
...
void ioStatePolling(){
  ...
  if (autoMode == false){
    ...
    unsigned long pollEndTime = millis();
    Serial.printf("Firebase polling time: %lu ms\n", pollEndTime - pollStartTime);
  }
...
void loadControl(){
  if (autoMode){ // Auto mode
    ...
    // Measure time for auto mode updates
    unsigned long autoStartTime = millis();
    ...
    unsigned long autoEndTime = millis();
    Serial.printf("Auto mode load control time: %lu ms\n", autoEndTime - autoStartTime);
  }
  else { // Manual mode
    unsigned long manualStartTime = millis();
    ...
    unsigned long manualEndTime = millis();
    Serial.printf("Manual load control execution: %lu ms\n", manualEndTime - manualStartTime);
  }
}

void loop() {
  Serial.print("In session: ");
  Serial.println(inSession);
...
// ========== Identify fingerprint ==========
  if (digitalRead(scannerIntPin) == HIGH && enrollFlag == false){
    ...
    Serial.printf("AutoIdentify: %lu ms\n", elapsedTime);
    timerStart(timer0);
    timerStart(timer1);
  }
// ========== Identify fingerprint ==========
